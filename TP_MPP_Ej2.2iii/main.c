...
#define FREQ_2HZ        2000
#define FREQ_10HZ       10000
#define FREQ_MODO_1HZ   1
#define FREQ_MODO_2HZ   2
#define FREQ_MODO_10HZ  3
...
uint16_t index = 0;
uint8_t dir_ant = 0;
uint8_t modo_freq = FREQ_MODO_1HZ;

int sec[] = { 180, -270, 135, -90, 45 };

static volatile uint16_t delay5ms = DELAY_xMS(1000);
static volatile uint16_t delay10ms = DELAY_xMS(10);
static volatile uint16_t delayFreqMotor = DELAY_xMS(1000);
...
void stepWithTimerA()
{
    if (!delayFreqMotor)
    {
        ...
        switch (modo_freq) {
            case FREQ_MODO_1HZ:
                delayFreqMotor = FREQ_MODO_1HZ;
                break;
            case FREQ_MODO_10HZ:
                delayFreqMotor = FREQ_MODO_10HZ;
                break;
            case FREQ_MODO_2HZ:
                delayFreqMotor = FREQ_MODO_2HZ;
                break;
            default:
                break;
        }
